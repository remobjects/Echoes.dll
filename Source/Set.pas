...
type
  ...
  private
    class var fConvertFrom: SetFunc<T,Integer>;
    var fValue: array of Byte;
    class constructor;
    method NGetEnumerator: IEnumerator; implements IEnumerable.GetEnumerator;
    class method EnumConverter(val: T): Integer;
    class method SByteConverter(val: T): Integer;
    class method UInt16Converter(val: T): Integer;
    class method Int16Converter(val: T): Integer;
    class method CharConverter(val: T): Integer;
    class method ByteConverter(val: T): Integer;
...
begin
  exit ClearRange(fConvertFrom(aStart), fConvertFrom(aEnd));
end;
...
begin
  exit SetRange(fConvertFrom(aStart), fConvertFrom(aEnd));
end;
...
begin
  exit aData.Get(fConvertFrom(aValue));
end;
...
class constructor &Set<T>;
begin
  var typeFromHandle: &Type := typeOf(T);
  if Activator.CreateInstance<T>() is &Enum then begin
    ...
    case typeFromHandle.FullName of
      ...
      'System.Int16': fConvertFrom := new SetFunc<T,Integer>(Int16Converter);
      'System.UInt16': fConvertFrom := new SetFunc<T,Integer>(UInt16Converter);
    else raise new NotSupportedException('Only enums, and small integer types are supported for Set<T>');
    end;
  end;
end;
...
class method &Set<T>.EnumConverter(val: T): Integer;
begin
  exit Convert.ToInt32(val);
end;

class method &Set<T>.SByteConverter(val: T): Integer;
begin
  exit Convert.ToInt32(val);
end;

class method &Set<T>.UInt16Converter(val: T): Integer;
begin
  exit Convert.ToInt32(val);
end;

class method &Set<T>.Int16Converter(val: T): Integer;
begin
  exit Convert.ToInt32(val);
end;

class method &Set<T>.CharConverter(val: T): Integer;
begin
  exit Convert.ToInt32(val);
end;

class method &Set<T>.ByteConverter(val: T): Integer;
begin
  exit Convert.ToInt32(val);
end;
